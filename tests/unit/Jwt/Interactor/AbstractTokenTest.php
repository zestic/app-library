<?php
declare(strict_types=1);

namespace Tests\Unit\Jwt\Interactor;

use App\Jwt\JwtConfiguration;

abstract class AbstractTokenTest
{
    protected function getExpectedPrivateKey(): string
    {
        return <<<PRIVATE_KEY
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----

PRIVATE_KEY;
    }

    protected function getExpectedPublicKey(): string
    {
        return <<<PUBLIC_KEY
-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----

PUBLIC_KEY;
    }

    protected function getJwtConfiguration(): JwtConfiguration
    {
        $config = [
            'algorithm' => 'RS256',
            'privateKey' => $this->getExpectedPrivateKey(),
            'publicKey' => $this->getExpectedPublicKey(),
            'tokenTtl' => 3600,
        ];

        return new JwtConfiguration($config);
    }

    protected function oldtoken(): string
    {
        return <<<TOKEN
eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.e30.Z2VBpRzUbCtvRbA7Ydsr9uaQnlgYmQBWkStH-mT0bUDQqLKcBoIzAKa-mRsOFWqdUGZMTIeClqZAtSwJx-4x4OD6mJqHPtqyhYi1s6rjuf747WrUxjrKbmJWN8k2tuDjOlLXlcR7H6C3KX5cGWZb9wUiR3jvuNYP_mEbaupv1MQHC_yE3I0zRPT1vFzJ8DEGfEFIxNisl5UIszcyAF1yjis6EzgZN5SKDGfmHzbAPOAshd83xzgqkCP9NmHq8QIkiAOdJm5DoKrTTgPUg9A7FPxb67mvUIXzu9PbtIjiCn3L8Qg2zM6-d6tDZ_RDvjlS9Qgbjii8qvn-DJuFilke0cMxhTPhF0nypy4uT-6iqKutkPN1Zak28gkJIkbN5EzA05BnlEOBDFhemKuLr35KS3UfQpNsSNuCpGjh3SPgRi0sSYe1X1E6KFlI58DD7d01BKbo5dbmW9xXM02zuTVG1pHCqhtiTbD8nlJL-OzRP1WEFH6wlw2ICaXHkQ3JHmFwWLyiMxcHgaRzbS4kbxWBdK9qCvf8umrt0CnSL7UBkzbAY4_J5MNDd8zBUcEauC-TaSjCbQzkz5wMYyM7uefHrekIUw1yHHYmYgR0eMf07Ehu6WuuZ6wTSFnX3EzlFVsb9tKUZ_00LmaZlaPex8gJzt4d-x3ARAHKjdTaRpTU744
TOKEN;
    }
}
